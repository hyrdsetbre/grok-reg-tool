...
## 邮件后端：对接 cloudflare_temp_email

本项目默认按 [dreamhunter2333/cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email) 作为邮件后端来设计。请先按照它的官方部署文档把 Cloudflare 临时邮箱服务配置好，包括 Cloudflare Email Routing、Workers、Pages、D1/KV 等它要求的基础配置；本 README 不重复搬运它的完整部署教程。

你只需要在 cloudflare_temp_email 能正常收信、能通过管理接口创建邮箱之后，把下面三个值填到 `docker/.env` 或 Web 控制台设置页：

| 本项目配置项 | 应填写的内容 |
| --- | --- |
| `MAIL_API_BASE` | cloudflare_temp_email 的后端 API 根地址，也就是 Worker 后端地址，例如 `https://你的后端域名`。不要填错成纯前端页面地址，除非你的前端域名已经正确代理到后端 API。 |
| `MAIL_ADMIN_AUTH` | cloudflare_temp_email 的网站 admin 密码。注册机创建邮箱时会把这个值作为 `x-admin-auth` 请求头调用管理接口。 |
| `MAIL_DOMAIN` | cloudflare_temp_email 中已经配置并可收信的域名，例如 `example.com`。注册机每轮会在这个域名下生成新邮箱地址。 |

本项目与邮件后端的调用关系大致如下：

1. 通过 `MAIL_API_BASE + /admin/new_address` 创建新邮箱地址。
2. 请求头中使用 `x-admin-auth: MAIL_ADMIN_AUTH`。
3. 请求体中使用 `MAIL_DOMAIN` 指定邮箱域名。
4. cloudflare_temp_email 返回邮箱地址和 `jwt`。
5. 注册机用这个 `jwt` 轮询邮件接口，提取 Grok/x.ai 发来的验证码。

如果邮件后端部署没完成，或 `MAIL_API_BASE`、`MAIL_ADMIN_AUTH`、`MAIL_DOMAIN` 任一项填错，注册流程通常会卡在“创建邮箱失败”或“获取验证码失败”。
...
邮件后端配置请优先参考上一节“对接 cloudflare_temp_email”。简单理解就是：`MAIL_API_BASE` 填后端地址，`MAIL_ADMIN_AUTH` 填后台 admin 密码，`MAIL_DOMAIN` 填已经配置好的收信域名。
...
- 感谢 [ReinerBRO/grok-register](https://github.com/ReinerBRO/grok-register)。本项目的 Grok 自动化注册思路和 Python 注册流程受该项目启发，并在此基础上整理为可自部署的 Web 控制台和 Docker 发布版。
- 感谢 [dreamhunter2333/cloudflare_temp_email](https://github.com/dreamhunter2333/cloudflare_temp_email)。本项目的邮件验证码获取流程默认面向该临时邮箱后端进行适配，用户可按其官方文档先部署好 Cloudflare 临时邮箱服务，再在本项目中填写后端地址、admin 密码和收信域名。
